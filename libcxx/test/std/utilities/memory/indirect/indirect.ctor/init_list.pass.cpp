...
constexpr void test_init_list_ctor_basic() {
  {
  ...
  }
  {
    const std::initializer_list<int> ilist {1, 2};
    int n = 0;
    std::indirect<S, test_allocator<S>> i(std::allocator_arg, test_allocator<S>(42), std::in_place, ilist, std::move(n));
    assert(!i.valueless_after_move());
