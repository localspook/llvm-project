...
void test_ctor_throws() {
  ...
  CopyCtorThrows c;
  try {
    std::indirect<CopyCtorThrows> i(std::allocator_arg, std::allocator<CopyCtorThrows>(), c);
    assert(false);
  } catch (const int& e) {
    assert(e == 42);
  } catch (...) {
...
// TODO
constexpr void test_in_place_t_ctor_basic() {
  {
    std::indirect<int, test_allocator<int>> i(std::allocator_arg, test_allocator<int>(42), 67);
